...
const fs = require('fs');
const {each, chain, reduce, uniqBy, filter} = require('lodash');
...
module.exports = function localizable(directory) {
    ...
    return chain(keys).groupBy('file').map(function (keys, file) {
        let _keys = filter(keys, key => ex.indexOf(key.key) === -1);
        if (!_keys.length)
            return "";
        return reduce(_keys, (acc, key) => {
